...
%%%%%%%%%LABERINTO FACIL NORMAL

map=im2bw(imread('laberinto_facil.bmp')); % input map read from a bmp file. for new maps write the file name here
source=[20 20]; % source position in Y, X format
goal=[700 350]; % goal position in Y, X format

%%PASO 50 25
stepsize= 50; % size of each step of the RRT
disTh = 25; % nodes closer than this threshold are taken as almost the same
...
%%%%%%%%LABERINTO FACIL AUMENTADO

% map=im2bw(imread('laberinto_facil_aumentado.bmp')); % input map read from a bmp file. for new maps write the file name here
% source=[41 41]; % source position in Y, X format
% goal=[1400 700]; % goal position in Y, X format
% 
% % stepsize= 120; % size of each step of the RRT
