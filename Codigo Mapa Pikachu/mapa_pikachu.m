...
% International License. The use of this code, its parts and all the
% materials in the text; creation of derivatives and their publication; and
% sharing the code publically is permitted without permission.

% Please cite the work in all materials as: R. Kala (2014) Code for Robot
% Path Planning using Rapidly-exploring Random Trees, Indian Institute of
% Information Technology Allahabad, Available at:
% http://rkala.in/codes.html


%Se debe descomentar el mapa que se quiere estudiar y comentar el que esta
%siendo estudiado

%Los mapas se encuentran por "secciones", esto solo representa los
...
%%%%%%%%%%%%OBSTACULOS FORMA PIKACHU NORMAL

map=im2bw(imread('mapa_pikachu.bmp')); % input map read from a bmp file. for new maps write the file name here
source=[140 200]; % source position in Y, X format
goal=[220 50]; % goal position in Y, X format 

stepsize= 150; % size of each step of the RRT
disTh = 70; % nodes closer than this threshold are taken as almost the same
maxFailedAttempts = 10000;
display = true; % display of RRTplay=t

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%OBSTACULOS FORMA PIKACHU AUMENTADO

% map=im2bw(imread('mapa_pikachu_aumentado.bmp')); % input map read from a bmp file. for new maps write the file name here
% source=[280 480]; % source position in Y, X format AUMENTADO
% goal=[500 100]; % goal position in Y, X format AUMENTADO
% 
% stepsize= 70; % size of each step of the RRT
