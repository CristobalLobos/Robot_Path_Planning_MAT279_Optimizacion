...
%%%%%%%%%%%LABERINTO CIRCULAR NORMAL

map=im2bw(imread('laberinto_circular.bmp')); % input map read from a bmp file. for new maps write the file name here
source=[165 190]; % source position in Y, X format
goal=[615 630]; % goal position in Y, X format
...
%%%%%%%LABERINTO CIRCULAR AUMENTADO
% 
% map=im2bw(imread('laberinto_circular_aumentado.bmp')); % input map read from a bmp file. for new maps write the file name here
% source=[330 380]; % source position in Y, X format
% goal=[1261 1230]; % goal position in Y, X format
% 
% stepsize= 70; % size of each step of the RRT
% disTh = 35; % nodes closer than this threshold are taken as almost the same
% maxFailedAttempts = 10000;
